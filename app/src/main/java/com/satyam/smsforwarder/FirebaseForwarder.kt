...
object FirebaseForwarder {
    ...
    private val callLogsRef = database.getReference("callLogs")
    ...
    fun sendCallLog(context: Context, number: String, type: String, duration: String) {
        initDevice(context)
        val timestamp = DateFormat.format("dd MMM yyyy, hh:mm a", Date()).toString()
        val timestampMillis = System.currentTimeMillis()
        ...
        callLogsRef.push().setValue(callLogData)
            .addOnSuccessListener {
                Log.d("FirebaseForwarder", "Call log uploaded to Firebase successfully")
            }
            .addOnFailureListener { e ->
                Log.e("FirebaseForwarder", "Failed to upload Call Log to Firebase", e)
            }
    }

    fun sendHistoricalCallLog(context: Context, number: String, type: String, duration: String, timestampMillis: Long) {
        initDevice(context)
        val timestamp = DateFormat.format("dd MMM yyyy, hh:mm a", Date(timestampMillis)).toString()
